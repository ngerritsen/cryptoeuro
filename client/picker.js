export default function initializePicker() {
  const pickerEl = getPickerEl();
...
function pickCurrency() {
  ...
  getCurrencies()
    .then(renderPicker)
    .then(() => {
      getPickerEl().querySelector('.js-form').addEventListener('submit', submit);
    });
}
...
function getPickerEl() {
  return document.querySelector('.js-picker');
}

function getCurrencies() {
  const currentPath = window.location.pathname;

  return fetch((currentPath === '/' ? '' : currentPath) + '/api/currencies')
    .then((response) => {
...
function getPickerButton() {
  return getPickerEl().querySelector('.js-picker-button');
}

function renderPicker(currencies) {
  getPickerEl().innerHTML = `
